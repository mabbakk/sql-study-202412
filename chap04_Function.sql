...
WHERE SN = '101'
    AND CHAR_4 = CHAR_6
;    -- 결과가 나오지 않는다는 건 FALSE.

-- CHAR 타입은 바이트 형식(10)이 맞지 않으면 10바이트가 아닌 문자열에 공백을 추가하여 형식을 맞춘다.
...
SELECT 
    'SQL' || 'Developer',   -- oracle에서만 문자열 결합 시 || 사용
...
SELECT
    LTRIM('xxYYZZxYZxx', 'x')   --x를 지우다가 다른 문자가 나오면 그만!
FROM dual;
...
SELECT 
    ...
    , ABS(-20) AS abs   -- 절대값
    , SIGN(99) AS sign  -- 0보다 작으면 -1, 0보다 크면 1, 0이면 0  , sign : 양수인지 음수인지 부호 판단!
FROM dual;

-- ## 날짜형 함수

-- 현재 날짜를 조회
SELECT SYSDATE   --현재 시간  (밀리초까지 나옴)
FROM dual;
...
SELECT 
    SYSDATE AS "현재 시간",
    SYSDATE - 1 AS "SYSDATE - 1",   -- 어제 날짜
    (SYSDATE + 10) - SYSDATE AS "날짜 - 날짜",   -- 10일 뒤 날짜 - 현재 날짜 = 사이 날짜
    SYSDATE - (1/24) AS "1시간 차감",
    SYSDATE - (1/24/60) * 100 AS "100분 차감",   --(1/24/60) = 1분 전
    SYSDATE - (1/24/60/60) * 30 AS "30초 차감"   --(1/24/60/60) = 1초
FROM dual;
...
SELECT
    ...
    TO_CHAR(SYSDATE + (1/24) * 6, 'YY/MM/DD HH24:MI:SS') AS "날짜와 시간", --MI = 분. 왜냐면 SQL은 자바와 다르게 대소문자 구분이 없기 때문에)
...
SELECT
    ...
    TO_CHAR(1350000, 'L999,999,999') AS "원화"   -- L : 각 나라의 원화 표현을 읽음
FROM dual;
...
SELECT 
    ...
    TO_DATE('20240101', 'YYYYMMDD'),   -- 시분초를 표현해주지 않으면 기본값 0시 0분 0초
    TO_DATE('202401','YYYYMM'),   --일월을 표현해주지 않으면 기본값 1월 1일     "
...
SELECT 
    ...
    CASE WHEN sal_cd = '100001' THEN '기본급여'
         ...
         ELSE '기타'
     END sal_name    -- 별칭 지정 AS였으나, 여기서는  END.
FROM tb_sal;
...
-- NVL2(expr1, expr2, expr3)   --oracle 전용 함수라 시험에 잘 안 나옴.
